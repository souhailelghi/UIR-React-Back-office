...
function DirecteurTable() {
  ...
  const handlePrint = async (documentId) => {
    try {
      ...
      const idTrainee = "46fe4f03-5e9e-4e10-2061-08dcc29f5788"; // Example trainee ID
      ...
      // Update InstantJSON
      const instantJSON = JSON.parse(document.instantJSON || "{}"); // Ensure instantJSON is an object
      // console.log('instantJSON : ', instantJSON);
  
      // Check if formFieldValues is missing or not an array
      if (!instantJSON.formFieldValues || !Array.isArray(instantJSON.formFieldValues)) {
        console.warn('formFieldValues is missing or not an array. Initializing as an empty array.');
        instantJSON.formFieldValues = []; // Initialize as an empty array
      }
      ...
      const updatedFormFieldValues = instantJSON.formFieldValues.map(field => {
        ...
        console.log("test traineeValue , ", traineeValue);
      ...
      console.log("test up : ", updatedFormFieldValues);
